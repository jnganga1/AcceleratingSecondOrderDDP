function Out = DDP_Regular_OldMethod(Nb,iLQR,N,rbtNmber,x0_diff) %'end' is 351 
...
A1 =  funcsOld.q(q_desired,qd_desired,u_desired);  
A2 =  funcsOld.qd(q_desired,qd_desired);  
A = [full(A1)';full(A2)']; 
A = eye(x_size,x_size) + [qd_x;A']*dt;
B = full(funcsOld.tau(q_desired))*dt;
B = [zeros(length(u)+1,length(u)); B(:,1:length(u))]; 


% [P,~,G] = dare(A,B,Q*dt*2,R*dt*2);
[P,~,G] = dare(A,B,Q,R);
% P(1,1)=P(1,1)*70;
...
%      ui = -.20*xi(ceil(length(xi)/2)+1:end-1); 


     ui = -.0025*(xi(ceil(length(xi)/2)+1:end-1)); 
%      ui = ones(1,Nb-1)*cntrl(i)* xi(ceil(length(xi)/2));
%        ui = cntrl(:,i);
...
while 1 == 1
    ...
    % Backward Pass 
    if params.iLQR
        [dV, Vx, Vxx, du, K] = BackwardPass_OldMethod(xbar, ubar, params);
        warning('Doing iLQR'); 
        BckTrkcnt=0;
