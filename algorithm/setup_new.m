function params = setup_new(x, u, l,Hmat,bVec,params,fPath)
    addpath(fPath);
    addpath([pwd '/Partials']);
    x_size = length(x);
    u_size = length(u);
...
%     lxx = hessian(l,x);
    lxx = double(simplify(jacobian(jacobian(l,x),x)));
    lux = double(simplify(jacobian(jacobian(l,u),x)));
    luu = double(simplify(jacobian(jacobian(l,u),u)));
